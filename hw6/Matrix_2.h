...
Michael Blumenshine
MPCS 51044
Homework 6, Problem 1 Matrix data structure
...
public:
	...
	Matrix(initializer_list<initializer_list<double>> init) {
		/** Note that the sizes of the initializer_list are checked against the rows and columns passed to the template.
		 * This ensures that the initializer_list will match the size of the matrix **/
		if ((init.size() != rows) || ((init.begin())->size() != cols))
		{
			// If they don't match, an invalid_argument exception is thrown.
	...
	double determinant() const {
		...
		// A static_assert is used to check that the columns and rows match. This is done at compile time.
